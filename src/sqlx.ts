...
export class Prepared
  implements SqlxPreparedQueriable<BindValue, SqliteQueryOptions> {
...
export class Queriable
  implements SqlxQueriable<BindValue, SqliteQueryOptions, Prepared> {
  readonly sqlxVersion = VERSION;
  readonly queryOptions: SqliteQueryOptions;
  /**
  ...
  prepare(sql: string, options?: SqliteQueryOptions | undefined): Prepared {
    return new Prepared(
      new Statement(
        this.handle,
        sql,
 ...
 */
export class Transaction extends Queriable implements
  ...
  > {
  constructor(handle: Deno.PointerValue, options?: SqliteQueryOptions) {
    super(options);
    this.handle = handle;
  }
 ...
 * const db = new SqliteConnection("myfile.db");
 *
 * // Open an in-memory database.
 * const db = new SqliteConnection(":memory:");
 *
 * // Open a read-only database.
 * const db = new SqliteConnection("myfile.db", { readonly: true });
 *
 * // Or open using File URL
 * const db = new SqliteConnection(new URL("./myfile.db", import.meta.url));
 * ```
 */
...
function transformToAsyncIterable<
  T extends unknown,
